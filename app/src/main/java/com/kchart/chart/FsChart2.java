...
public final class FsChart2 extends ChartBase {

    private static final String TAG="FsChart";

    private String time[];

    public FsChart2(Context context) {
        super(context);
    }

    public FsChart2(Context context, AttributeSet attrs) {
        super(context, attrs);
    }
    ...
    public void onDraw(Canvas canvas) {
        ...
        if(base != null && chartData != null) {
            drawGrid(); // 画分时图成交量外面的方框
            drawHQChart(); // 画分时线、均线、成交量竖线
            drawPrice(); // 画左侧的5个价格，右侧的5个涨跌幅和底部的时间
            drawCursorLine(); // 画光标
            drawMidText1(); // 画横屏查看更多
        } else {
            drawGrid();
        }
    }
    ...
    protected void initDataParam() {
        super.initDataParam();
        time=(new String[]{"9:30", "11:30","13:00", "14:30","16:00"});
    ...
    private void drawHQChart() {
        // Color.parseColor("#31302f")
        if(chartData != null) {
            chartPaint.setColor(ColorScheme._HQ_CJL_COLOR); // 阴影颜色
            drawChartLine(chartPaint, indicatorRect, "volume", m_maxCJLBL, 0.0F, 242, 1, true);
            chartPaint.setColor(Color.parseColor("#6783b0")); // 分时线
            drawChartLine(chartPaint, chartRect, "price", m_maxPrice, m_minPrice, 242, 2);
            volumePaint.setColor(Color.parseColor("#edad33")); // 均线
            drawChartLine(volumePaint, chartRect, "avg_price", m_maxPrice, m_minPrice, 242, 0);
        }
    }
